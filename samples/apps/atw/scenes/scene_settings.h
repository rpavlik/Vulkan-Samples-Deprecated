...
static const int displayResolutionTable[] = {1920, 1080, 2560, 1440, 3840, 2160, 7680, 4320};

static const int eyeResolutionTable[] = {1024, 1536, 2048, 4096};

static const ksGpuSampleCount eyeSampleCountTable[] = {KS_GPU_SAMPLE_COUNT_1, KS_GPU_SAMPLE_COUNT_2, KS_GPU_SAMPLE_COUNT_4,
...
static void ksSceneSettings_Init(ksGpuContext* context, ksSceneSettings* settings) {
    ...
    settings->maxDisplayResolutionLevels =
        (!ksGpuWindow_SupportedResolution(displayResolutionTable[1 * 2 + 0], displayResolutionTable[1 * 2 + 1])
             ? 1
             : (!ksGpuWindow_SupportedResolution(displayResolutionTable[2 * 2 + 0], displayResolutionTable[2 * 2 + 1])
                    ? 2
                    : (!ksGpuWindow_SupportedResolution(displayResolutionTable[3 * 2 + 0], displayResolutionTable[3 * 2 + 1])
                           ? 3
                           : 4)));
    ...
    settings->maxEyeImageSamplesLevels =
        (maxEyeImageSamplesLevels < MAX_EYE_IMAGE_SAMPLES_LEVELS) ? maxEyeImageSamplesLevels : MAX_EYE_IMAGE_SAMPLES_LEVELS;
}
...
static void ksSceneSettings_CycleDisplayResolutionLevel(ksSceneSettings* settings) {
    CycleLevel(&settings->displayResolutionLevel, settings->maxDisplayResolutionLevels);
}
static void ksSceneSettings_CycleEyeImageResolutionLevel(ksSceneSettings* settings) {
    CycleLevel(&settings->eyeImageResolutionLevel, settings->maxEyeImageResolutionLevels);
}
static void ksSceneSettings_CycleEyeImageSamplesLevel(ksSceneSettings* settings) {
    CycleLevel(&settings->eyeImageSamplesLevel, settings->maxEyeImageSamplesLevels);
}

static void ksSceneSettings_CycleDrawCallLevel(ksSceneSettings* settings) {
    CycleLevel(&settings->drawCallLevel, MAX_SCENE_DRAWCALL_LEVELS);
}
static void ksSceneSettings_CycleTriangleLevel(ksSceneSettings* settings) {
    CycleLevel(&settings->triangleLevel, MAX_SCENE_TRIANGLE_LEVELS);
}
static void ksSceneSettings_CycleFragmentLevel(ksSceneSettings* settings) {
    CycleLevel(&settings->fragmentLevel, MAX_SCENE_FRAGMENT_LEVELS);
}

static void ksSceneSettings_SetDisplayResolutionLevel(ksSceneSettings* settings, const int level) {
    settings->displayResolutionLevel =
        (level < settings->maxDisplayResolutionLevels) ? level : settings->maxDisplayResolutionLevels;
}
static void ksSceneSettings_SetEyeImageResolutionLevel(ksSceneSettings* settings, const int level) {
    settings->eyeImageResolutionLevel =
        (level < settings->maxEyeImageResolutionLevels) ? level : settings->maxEyeImageResolutionLevels;
}
static void ksSceneSettings_SetEyeImageSamplesLevel(ksSceneSettings* settings, const int level) {
    settings->eyeImageSamplesLevel = (level < settings->maxEyeImageSamplesLevels) ? level : settings->maxEyeImageSamplesLevels;
}

...
static void ksViewState_DerivedData(ksViewState* viewState, const ksMatrix4x4f* centerViewMatrix) {
    ...
    combinedProjectionMatrix.m[0][0] =
        viewState->projectionMatrix[0].m[0][0] / (fabsf(viewState->projectionMatrix[0].m[2][0]) + 1.0f);
    combinedProjectionMatrix.m[2][0] = 0.0f;

    ksMatrix4x4f moveBackMatrix;
    ksMatrix4x4f_CreateTranslation(&moveBackMatrix, 0.0f, 0.0f,
                                   -0.5f * viewState->interpupillaryDistance * combinedProjectionMatrix.m[0][0]);
...
static void ksViewState_Init(ksViewState* viewState, const float interpupillaryDistance) {
    ...
    for (int eye = 0; eye < NUM_EYES; eye++) {
        ...
        ksMatrix4x4f_CreateProjectionFov(&viewState->projectionMatrix[eye], 45.0f, 45.0f, 30.0f, 30.0f, DEFAULT_NEAR_Z,
                                         INFINITE_FAR_Z);
...
static void ksViewState_HandleInput(ksViewState* viewState, ksGpuWindowInput* input, const ksNanoseconds time) {
    ...
    if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_SHIFT_LEFT)) {
        if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_UP)) {
            rotationDelta.x -= ROTATION_DEGREES_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_DOWN)) {
            rotationDelta.x += ROTATION_DEGREES_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_LEFT)) {
            rotationDelta.y += ROTATION_DEGREES_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_RIGHT)) {
            rotationDelta.y -= ROTATION_DEGREES_PER_TAP;
        }
    } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CTRL_LEFT)) {
        if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_UP)) {
            translationDelta.y += TRANSLATION_UNITS_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_DOWN)) {
            translationDelta.y -= TRANSLATION_UNITS_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_LEFT)) {
            translationDelta.x -= TRANSLATION_UNITS_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_RIGHT)) {
            translationDelta.x += TRANSLATION_UNITS_PER_TAP;
        }
    } else {
        if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_UP)) {
            translationDelta.z -= TRANSLATION_UNITS_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_DOWN)) {
            translationDelta.z += TRANSLATION_UNITS_PER_TAP;
        } else if (ksGpuWindowInput_CheckKeyboardKey(input, KEY_CURSOR_LEFT)) {
            rotationDelta.y += ROTATION_DEGREES_PER_TAP;
    ...
    ksMatrix4x4f_CreateTranslation(&viewTranslation, -viewState->viewTranslation.x, -viewState->viewTranslation.y,
                                   -viewState->viewTranslation.z);
...
static void ksViewState_HandleHmd(ksViewState* viewState, const ksNanoseconds time) {
    ...
    for (int eye = 0; eye < NUM_EYES; eye++) {
        ...
        ksMatrix4x4f_CreateProjectionFov(&viewState->projectionMatrix[eye], 45.0f, 45.0f, 36.0f, 36.0f, DEFAULT_NEAR_Z,
                                         INFINITE_FAR_Z);
    }
